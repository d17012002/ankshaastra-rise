...
const YouTubeSection = () => {
  ...
  return (
    <section className="py-20 md:py-28 relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #2E1A47 0%, #0F0E1A 100%)' }}>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {videos.map((video, index) => (
              <div 
                key={index}
                className="relative bg-white/5 backdrop-blur-xl rounded-2xl p-4 border border-accent/30 gold-glow group hover:border-accent/50 transition-all duration-300"
              >
                ...
                {/* Video Title */}
                <h3 className="text-white font-semibold text-center group-hover:text-accent transition-colors">
                  {video.title}
                </h3>
              </div>
            ))}
